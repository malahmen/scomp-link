...
- Commands: `install-deps`, `configure`, `start`, `stop`, `status`, `edit`, `create-account`, `list-accounts`, `delete-account`, `search`, `build-image`, `run-docker`, `run-k8s`. The interactive menu groups these into categories (Setup, Local, Deploy, Accounts, Search, Status) rather than one flat list, direct CLI invocation is unaffected
