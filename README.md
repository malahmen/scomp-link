...
- **Warden anti-cheat modules**: baked into the Docker/K8s image at build time (small and static like the compiled binaries, unlike the large, runtime-mounted game data), and pointed at directly from the repack for local native. Without this, Warden still runs (it's enabled by default) but has nothing to scan with, which surfaces as players getting kicked mid-session for a timeout, not just a log warning at startup
...
- Commands: `install-deps`, `configure`, `start`, `stop`, `status`, `edit`, `create-account`, `list-accounts`, `delete-account`, `build-image`, `run-docker`, `run-k8s`
