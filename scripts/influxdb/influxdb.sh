#!/usr/bin/env bash
# description: InfluxDB 2.x time-series database (Docker/K8s)
...
check_dependencies() {
    ...
    case "$TARGET_TYPE" in
        ...
        kind|k8s) _check_kubectl; _ensure_helm "InfluxDB" ;;
    esac
}
...
influxdb_connect_k8s() {
    ...
    if pf_is_running "$_IDB_PF_PID"; then
        gum style --foreground "$GREEN" --border-foreground "$GREEN" --border rounded \
            --width 60 --margin "0 2" --padding "1 2" \
            "Port-forward is running" \
            "http://localhost:$(pf_port "$_IDB_PF_PID")"
        gum confirm "Stop port-forward?" && pf_stop "$_IDB_PF_PID" || true
        return
    fi
...
_k8s_menu() {
    ...
    while true; do
        ...
        local pf_label
        pf_is_running "$_IDB_PF_PID" \
            && pf_label="connect  [● localhost:$(pf_port "$_IDB_PF_PID")]" \
