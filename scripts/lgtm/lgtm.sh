...
# Docker config files──
# The compose file above points every component at a config bind-mounted from
# ${HOSTPATH_BASE}/<component>/config (and Grafana at .../provisioning), but
# nothing used to generate those files, so the containers crash-looped on start.
# Write minimal single-node configs for the selected components. Existing files
# are never overwritten so user customisations survive a re-run of install.

# Usage: _write_config_file <path> <label>   (content on stdin)
# Skips (and drains stdin) when the file already exists.
_write_config_file() {
    local path="$1" label="$2"
    if [[ -f "$path" ]]; then
        info "${label}: keeping existing ${path}"
        cat >/dev/null
        return 0
    fi
    mkdir -p "$(dirname "$path")"
    cat > "$path"
    success "${label}: wrote ${path}"
}

_write_docker_configs() {
    local enabled="$1"
    local base_path
    base_path=$(cfg_get "HOSTPATH_BASE")
    ...
    for c in $enabled; do
        ...
        case "$c" in
            loki)
                # Single-binary Loki 3.x: filesystem storage, in-memory ring, TSDB v13 schema.
                _write_config_file "${data_path}/config/local-config.yaml" "Loki config" <<EOF
# Auto-generated by lgtm.sh — edit freely, install will not overwrite this file.
auth_enabled: false
...
EOF
                ;;
            tempo)
                # Tempo 2.x monolithic: OTLP receivers, local block storage under /var/tempo.
                _write_config_file "${data_path}/config/tempo.yaml" "Tempo config" <<EOF
# Auto-generated by lgtm.sh — edit freely, install will not overwrite this file.
server:
...
EOF
                ;;
            mimir)
                # Mimir monolithic (-target=all): filesystem storage under /data,
                # single-node memberlist rings, multitenancy off.
                _write_config_file "${data_path}/config/mimir.yaml" "Mimir config" <<EOF
# Auto-generated by lgtm.sh — edit freely, install will not overwrite this file.
target: all
...
EOF
                ;;
            otelcol)
                # Pipelines are only wired to components present in the stack.
                local exporters="" traces_pipeline="" logs_pipeline="" metrics_pipeline=""
                if [[ " $enabled " == *" tempo "* ]]; then
                    exporters+=$'\n  otlp/tempo:\n    endpoint: tempo:4317\n    tls:\n      insecure: true'
                    traces_pipeline=$'\n    traces:\n      receivers: [otlp]\n      exporters: [otlp/tempo]'
                fi
                if [[ " $enabled " == *" loki "* ]]; then
                    # 'loki' exporter matches the pinned contrib image (removed in ≥0.131,
                    # see the note in _helm_values_otelcol).
                    exporters+=$'\n  loki:\n    endpoint: http://loki:3100/loki/api/v1/push'
                    logs_pipeline=$'\n    logs:\n      receivers: [otlp]\n      exporters: [loki]'
                fi
                if [[ " $enabled " == *" mimir "* ]]; then
                    exporters+=$'\n  prometheusremotewrite:\n    endpoint: http://mimir:9009/api/v1/push'
                    metrics_pipeline=$'\n    metrics:\n      receivers: [otlp]\n      exporters: [prometheusremotewrite]'
                fi
                if [[ -z "$exporters" ]]; then
                    # No backend selected: keep the collector alive with a debug sink.
                    exporters=$'\n  debug: {}'
                    traces_pipeline=$'\n    traces:\n      receivers: [otlp]\n      exporters: [debug]'
                fi

                _write_config_file "${data_path}/config/config.yaml" "OTel Collector config" <<EOF
# Auto-generated by lgtm.sh — edit freely, install will not overwrite this file.
receivers:
  otlp:
    protocols:
...
service:
  pipelines:${traces_pipeline}${logs_pipeline}${metrics_pipeline}
EOF
                ;;
            grafana)
                # Grafana replaces its whole provisioning dir with the bind mount, so
                # create the sibling folders it expects to find (avoids startup warnings).
                local sub
                for sub in datasources dashboards plugins notifiers alerting; do
                    mkdir -p "${data_path}/provisioning/${sub}"
                done
                ...
                [[ -z "$datasources" ]] && datasources=" []"

                _write_config_file "${data_path}/provisioning/datasources/lgtm.yaml" "Grafana datasources" <<EOF
# Auto-generated by lgtm.sh — edit freely, install will not overwrite this file.
apiVersion: 1
...
_install_docker() {
    ...
    # Component configs + Grafana datasource provisioning (skips existing files)
    _write_docker_configs "$enabled"
