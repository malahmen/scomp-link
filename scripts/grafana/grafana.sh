...
_ensure_helm() {
    ...
    gum style \
        ...
        "helm not found" \
        "helm is required to install Grafana on Kubernetes."
...
_ensure_helm_repo() {
    info "Adding/updating '${GRF_HELM_REPO_NAME}' helm repo..."
    helm repo add "$GRF_HELM_REPO_NAME" "$GRF_HELM_REPO_URL" 2>/dev/null || true
    gum spin --spinner dot --title "Updating helm repo..." -- \
        helm repo update "$GRF_HELM_REPO_NAME"
}
