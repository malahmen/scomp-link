#!/usr/bin/env bash
# description: MongoDB: install, connect, manage (Docker/K8s)
...
check_dependencies() {
    ...
    case "$TARGET_TYPE" in
        ...
        kind|k8s) _check_kubectl; _ensure_helm "MongoDB" ;;
    esac
}
...
# Host-side directory holding the per-container init script that creates the
# app user (bind-mounted read-only to /docker-entrypoint-initdb.d).
_docker_init_dir() {
    echo "${XDG_CONFIG_HOME:-$HOME/.config}/scomp-link/mongodb/${MNG_CONTAINER_NAME}/init"
}

# Emits "$1" as a JSON/JS double-quoted string literal. Backslashes and double
# quotes are escaped; values containing control characters are rejected (rc 1).
_js_quote() {
    local s="$1"
    [[ "$s" == *[[:cntrl:]]* ]] && return 1
    s="${s//\\/\\\\}"
...
# Writes create-app-user.js into the init dir and prints the dir. The official
# mongo image runs *.js files from /docker-entrypoint-initdb.d (authenticated as
# root, `db` bound to MONGO_INITDB_DATABASE) on the first start with an empty
# data dir. Returns 1 without writing if a value cannot be safely quoted.
_docker_write_init_script() {
    local init_dir q_db q_user q_pass
    init_dir=$(_docker_init_dir)
    q_db=$(_js_quote "$MNG_DB")         || return 1
    q_user=$(_js_quote "$MNG_USER")     || return 1
    q_pass=$(_js_quote "$MNG_PASSWORD") || return 1

    mkdir -p "$init_dir"
    cat > "${init_dir}/create-app-user.js" <<EOF
// Generated by scomp-link mongodb.sh — creates the application user on first init.
db.getSiblingDB(${q_db}).createUser({
  user: ${q_user},
  pwd: ${q_pass},
  roles: [{ role: "readWrite", db: ${q_db} }]
});
EOF
    echo "$init_dir"
}
...
mongodb_install_docker() {
    ...
    # App user: the image only creates the root user from env, so the app user is
    # created by an init script on first start. Its credentials are also kept in
    # the container env (alongside MONGO_INITDB_ROOT_*) so connect can reuse them.
    local init_dir=""
    if [[ -n "$MNG_USER" && -n "$MNG_PASSWORD" ]]; then
        if init_dir=$(_docker_write_init_script); then
            docker_args+=(
                -e SCOMP_APP_USERNAME="$MNG_USER"
                -e SCOMP_APP_PASSWORD="$MNG_PASSWORD"
                -v "${init_dir}:/docker-entrypoint-initdb.d:ro"
            )
            if docker volume inspect "$volume" &>/dev/null 2>&1; then
                warn "Volume '${volume}' already exists — init scripts only run on an empty data dir,"
                warn "so app user '${MNG_USER}' will not be (re)created. Remove the volume for a fresh init."
            fi
        else
            init_dir=""
            warn "Database/username/password contain control characters — skipping app user init script."
        fi
    fi
    ...
    gum style \
        ...
        "Volume:     ${volume}"
    [[ -n "$init_dir" ]] && info "App user init script: ${init_dir}/create-app-user.js"
}
...
mongodb_connect_docker() {
    ...
    local env_vars root_user root_pass db app_user app_pass
    ...
    app_user=$(echo  "$env_vars" | grep '^SCOMP_APP_USERNAME='          | cut -d= -f2- || echo "")
    app_pass=$(echo  "$env_vars" | grep '^SCOMP_APP_PASSWORD='          | cut -d= -f2- || echo "")

    # Prefer the app user (created by the init script in the app database, so it
    # authenticates against that database); fall back to root when none is set.
    local user pass auth_db
    if [[ -n "$app_user" && -n "$app_pass" ]]; then
        user="$app_user"; pass="$app_pass"; auth_db="$db"
    else
        user="$root_user"; pass="$root_pass"; auth_db="admin"
    fi

    info "Connecting as ${user} → ${db} (auth db: ${auth_db}). Type exit or quit() to quit."
    echo ""

    docker exec -it "$MNG_CONTAINER_NAME" \
        "$cli" -u "$user" -p "$pass" \
        --authenticationDatabase "$auth_db" "$db"
}
...
mongodb_uninstall_docker() {
    ...
    if [[ "$remove_volume" == true ]]; then
        ...
        # The init script only matters for a fresh data dir; drop it with the volume.
        if [[ -d "$init_dir" ]]; then
            rm -rf "$init_dir"
            rmdir "$(dirname "$init_dir")" 2>/dev/null || true
        fi
        success "Container, volume and app user init script removed."
    else
...
mongodb_port_forward_k8s() {
    ...
    if pf_is_running "$_MNG_PF_PID"; then
        gum style --foreground "$GREEN" --border-foreground "$GREEN" --border rounded \
            --width 60 --margin "0 2" --padding "1 2" \
            "Port-forward is running" \
            "localhost:$(pf_port "$_MNG_PF_PID") → ${MNG_HELM_RELEASE}:27017" \
            "" \
            "Connect: mongosh --port $(pf_port "$_MNG_PF_PID") -u <user> --authenticationDatabase <admin|app db>"
        gum confirm "Stop port-forward?" && pf_stop "$_MNG_PF_PID" || true
        return
    fi
    ...
    info "Connect: mongosh --port ${port} -u <user> --authenticationDatabase <admin for root, the app database for the app user>"
}
...
mongodb_connect_k8s() {
    ...
    # The chart creates the app user inside the app database (userDatabase.*),
    # while the root user lives in admin — so the auth database depends on who
    # is logging in. Default accordingly, but let the user override.
    local auth_default="$db"
    [[ "$user" == "$MNG_DEFAULT_ROOT_USER" || "$user" == "root" ]] && auth_default="admin"
    local auth_db
    auth_db=$(gum input \
        --placeholder "$auth_default" \
        --header "Authentication database (root → admin, app user → its database; leave empty for '${auth_default}'):") || true
    auth_db="${auth_db:-$auth_default}"
    ...
    "$client" --host 127.0.0.1 --port "$port" \
        -u "$user" --authenticationDatabase "$auth_db" "$db" || true
...
_k8s_menu() {
    ...
    while true; do
        ...
        local pf_label
        pf_is_running "$_MNG_PF_PID" \
            && pf_label="port-forward  [● localhost:$(pf_port "$_MNG_PF_PID")]" \
