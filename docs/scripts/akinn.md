...
- **Two hand-off modes** - **run** the assembled command here (Akinn self-elevates with `sudo` and provisions this node) or **print** it to paste on the target node yourself. The self-elevation re-execs through `sh`, so it works on a fresh, non-executable clone with current Akinn; an older cached clone `exec`s `$0` directly and fails there - accept the TUI's `git pull` offer (or run it with `sudo sh akinn.sh …` yourself).
- **Standalone-friendly** - experienced users can skip the TUI entirely and call `sh akinn.sh -m node1 -v v1.34 -c v3.32.2 -n 10.244.0.0/16` directly (`-c` must be a Calico tag that exists on its releases page - Akinn validates it there); the TUI only helps build that command. Akinn itself stays gum-free `/bin/sh` so it runs on a bare node with no extra dependencies.
